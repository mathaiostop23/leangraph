...
pub fn propose(
    ...
    let branch = format!("arbor/issue-{issue_number}");
    ...
    let work: PathBuf = repo_dir
        .parent()
        .unwrap_or(repo_dir)
        .join(format!(".arbor-fix-{issue_number}"));
    ...
    let result = (|| -> Result<Proposal> {
        let patch_path = work.join(".arbor-patch");
        std::fs::write(&patch_path, patch)?;

        // Check before apply, so a bad patch leaves the worktree untouched.
        git(&work, None, &["apply", "--check", "--whitespace=nowarn", ".arbor-patch"])
            .context("the patch does not apply cleanly to the current tree")?;
        git(&work, None, &["apply", "--whitespace=nowarn", ".arbor-patch"])?;
        ...
        git(
            ...
            &[
                ...
                "user.name=arbor",
                "-c",
                "user.email=arbor@localhost",
                "commit",
                "-m",
                &format!("Proposed fix for #{issue_number}\n\nGenerated by arbor. Review before merging."),
            ],
        )?;
...
pub async fn open_pr(
    ...
    let res = reqwest::Client::new()
        ...
        .json(&serde_json::json!({
            ...
            "base": base,
            "body": format!(
                "Proposed by arbor for #{issue_number}.\n\n\
