...
/// Python's standard library, top level only.
///
/// Only the names a repository plausibly collides with — this is a shadowing
/// check, not a completeness exercise, and a name missing from here costs one
/// false edge rather than breaking anything.
const PY_STDLIB: &[&str] = &[
    "abc",
    "argparse",
    "array",
...
impl Lang {
    ...
    /// Would a bare module of this name be the language's own, not the repo's?
    ///
    /// A repository file can share a name with a standard library module —
    /// `json.py`, `io.py`, `types.py` are common — and the suffix keys make it
    /// answerable by that bare name. Then `import json` binds to it. Only the
    /// single-segment key is at risk: nothing in the standard library is
    /// addressed as `core.serializers.json`.
    pub fn shadows_stdlib(self, name: &str) -> bool {
        match self {
            Lang::Python => PY_STDLIB.contains(&name),
            _ => false,
        }
    }
