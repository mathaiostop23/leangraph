...
ARBOR_ANTHROPIC_BASE="http://127.0.0.1:$STUB_A/v1/messages" \
ARBOR_GITHUB_API="http://127.0.0.1:$STUB_G" \
ARBOR_ANTHROPIC_KEY=sk-stub \
ARBOR_GITHUB_TOKEN=ghp-stub \
ARBOR_WEBHOOK_SECRET=$SECRET \
ARBOR_MASTER_KEY=$(printf '0%.0s' {1..64}) \
  target/release/arbor server --addr "127.0.0.1:$PORT" --data "$DATA" --workers 2 \
...
print(json.dumps({"action":"labeled","repository":{"full_name":"pallets/flask"},
 ...
 "author_association":"OWNER","labels":[{"name":"arbor"}]}}))' "$1" "$2" "$3")
